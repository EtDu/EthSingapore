...
//address mapper contract and transfer gateway
...
async function depositCoinToRinkebyGateway(web3js, amount, ownerAccount, gas) {
    ...
    let gasEstimate = await contract.methods
        .approve(gateway._address, amount)
    ...
    gasEstimate = await gateway.methods
        .depositERC20(amount, contract._address)
        .estimateGas({ from: ownerAccount, gas })
...
const gateway_api = {
    ...
    deposit_coin: async (amount, gas) => {
        const { account, web3js } = loadRinkeyAccount()
        try {
            const tx = await depositCoinToRinkebyGateway(
                web3js, amount * coinMultiplier, account.address, gas || 350000
            )
    ...
    withdraw_coin: async (amount, gas, timeout) => {
        let client
        try {
            ...
            const signature = await depositCoinToExtdevGateway({
                ...
                timeout: timeout ? (timeout * 1000) : 120000
            })
    ...
    withdraw_token: async (uid, gas, timeout) => {
        let client
        try {
            const extdev = loadExtdevAccount()
            const rinkeby = loadRinkeyAccount()
    ...
    coin_balance: async (chain, address) => {
        try {
            ...
            if (chain === 'eth') {
                ...
                if (address) {
                    ownerAddress = (address === 'gateway') ? rinkebyGatewayAddress : address
                }
                ...
                if (address) {
                    ownerAddress = (address === 'gateway') ? extdevGatewayAddress : address
                }
                try {
    ...
    deposit_token: async (uid, gas) => {
        const { account, web3js } = loadRinkeyAccount()
        try {
            const tx = await depositTokenToGateway(web3js, uid, account.address, gas || 350000)
            console.log(`Token ${uid} deposited, Rinkeby tx hash: ${tx.transactionHash}`)
            return {uid: uid, txhash: tx.transactionHash}
        } catch (err) {
            console.error(err)
        }
    },
    ...
    mint_token: async (uid, gas) => {
        const { account, web3js } = loadRinkeyAccount()
        try {
            const tx = await mintToken(web3js, uid, account.address, gas || 350000)
    ...
    token_balance: async (chain, address) => {
        try {
            let ownerAddress, balance
            if (chain === 'eth') {
                const { account, web3js } = loadRinkeyAccount()
    ...
    map_contracts: async (contractType, options) => {
        let client
        try {
            const rinkeby = loadRinkeyAccount()
            const extdev = loadExtdevAccount()
...
program.command('deposittoken').description('calls deposit_token').action(function() {
    gateway_api.deposit_token(2, 350000).then(function(result){
